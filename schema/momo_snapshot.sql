...
CREATE OR REPLACE FUNCTION "public"."upsert_category_rule"("p_user_id" "uuid", "p_household_id" "uuid", "p_normalized_text" "text", "p_category" "text") RETURNS "void"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'pg_catalog', 'public'
    AS $$
BEGIN
...
CREATE UNIQUE INDEX "idx_category_rules_household_text" ON "public"."category_rules" USING "btree" ("household_id", "normalized_text") WHERE ("household_id" IS NOT NULL);



CREATE UNIQUE INDEX "idx_category_rules_personal_text" ON "public"."category_rules" USING "btree" ("user_id", "normalized_text") WHERE ("household_id" IS NULL);
...
CREATE POLICY "category_rules_insert" ON "public"."category_rules" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = ( SELECT "auth"."uid"() AS "uid")) AND (("household_id" IS NULL) OR "public"."is_member_definer_uid"("household_id", ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "category_rules_select" ON "public"."category_rules" FOR SELECT TO "authenticated" USING (((("household_id" IS NULL) AND ("user_id" = ( SELECT "auth"."uid"() AS "uid"))) OR (("household_id" IS NOT NULL) AND "public"."is_member_definer_uid"("household_id", ( SELECT "auth"."uid"() AS "uid")))));
